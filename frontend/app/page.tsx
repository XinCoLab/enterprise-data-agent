...
type ChatResponse = { status: "success" | "incomplete"; thread_id: string; model: Model; latency_ms: number; answer: string; tool_counts: Record<string, number>; sql_queries: { tool_call_id: string; sql: string; result?: SqlResult }[]; result_preview?: SqlResult | null; knowledge_view?: { knowledge_view_mode?: string } | null };
...
function ResultTable({ result }: { result?: SqlResult | null }) {
  ...
  return <div className="result-block"><div className="result-heading"><strong>查询结果</strong><span>{result.returned_rows ?? result.rows.length} 行{result.truncated ? " · 已截断" : ""}</span></div><div className="table-scroll"><table><thead><tr>{result.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{result.rows.map((row, rowIndex) => <tr key={rowIndex}>{result.columns!.map((column) => <td key={column}>{String(row[column] ?? "NULL")}</td>)}</tr>)}</tbody></table></div></div>;
}
...
export default function Home() {
  ...
  const sendQuestion = async () => {
    ...
    try {
      const response = await api<ChatResponse>("/api/chat", { method: "POST", body: JSON.stringify({ question: text, thread_id: threadId, model }) });
      setThreadId(response.thread_id);
      setMessages((current) => [...current, { id: crypto.randomUUID(), role: "assistant", content: response.answer || "分析完成，但没有生成可展示的回答。", details: response }]);
    } catch (error) {
  ...
  return <div className="product-shell">
    ...
    <main className="main-area"><header className="app-header"><div><h1>{pageNames[page]}</h1><p>{backendName(state.active.backend)} · {state.active.database || "本地文件"}</p></div><div className="header-actions">
      <label className="model-select"><span>模型</span><select value={model} onChange={(event) => setModel(event.target.value as Model)}>{state.models.map((item) => <option key={item} value={item}>{modelName(item)}</option>)}</select></label>
      {page === "analysis" && <button className="button secondary" onClick={newConversation}>新会话</button>}
    </div></header>
    ...
    {page === "analysis" && <section className="analysis-page"><div className="conversation">
      {messages.length === 0 ? <div className="empty-state"><h2>开始一次数据分析</h2><p>输入业务问题。Agent 会查找已配置的 Knowledge，执行只读 SQL，再返回结果。</p></div> : messages.map((message) => <article className={`message ${message.role}`} key={message.id}>
        ...
        {message.details && <><ResultTable result={message.details.result_preview} /><details className="run-details"><summary>查看 SQL 与运行信息</summary><div className="metric-row"><span>{(message.details.latency_ms / 1000).toFixed(1)} 秒</span><span>{message.details.sql_queries.length} 次 SQL</span><span>{message.details.knowledge_view?.knowledge_view_mode || "-"} View</span>{message.details.status === "incomplete" && <span className="warning-text">未完成</span>}</div>{message.details.sql_queries.map((query, queryIndex) => <div className="sql-card" key={query.tool_call_id || queryIndex}><div>SQL {queryIndex + 1}</div><pre><code>{query.sql}</code></pre></div>)}</details></>}
      </article>)}{chatBusy && <article className="message assistant pending"><div className="message-label">DataAgent</div><p>正在分析…</p></article>}
    </div><div className="composer"><textarea value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendQuestion(); } }} placeholder="输入数据分析问题，Enter 发送，Shift + Enter 换行" /><button className="send-button" disabled={chatBusy || !question.trim()} onClick={sendQuestion}>发送</button></div></section>}
