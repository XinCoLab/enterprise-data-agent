...
export default function KnowledgeGraph({ revision }: { revision: number }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const shellRef = useRef<HTMLDivElement>(null);
  const drawRef = useRef<() => void>(() => undefined);
  const controllerRef = useRef<{ zoom: (factor: number) => void; reset: () => void } | null>(null);
  ...
  useEffect(() => {
    ...
    const resetView = () => {
      ...
      transform.scale = Math.min(1.15, Math.max(0.72, width / 920));
      draw();
    };
    ...
    function draw() {
      ...
      edges.forEach((edge) => {
        ...
        context.strokeStyle = active ? "rgba(37,99,235,.62)" : "rgba(100,116,139,.22)";
      ...
      nodes.forEach((node) => {
        ...
        const radius = (6.5 + typeBoost + Math.min(7, Math.sqrt(node.degree) * 1.25)) * transform.scale;
        ...
        const showLabel = selected || hovered || node.knowledge_type === "table" || (activeQuery && matches);
        if (showLabel) {
    ...
    const simulate = () => {
      ...
      draw();
      if (frame < 190 || draggedNode || isPanning) animationFrame = requestAnimationFrame(simulate);
    };
  ...
  return <section className="knowledge-graph-panel">
    <div className="knowledge-graph-heading">
      <div><h2>Knowledge Graph</h2><p>{payload.nodes.length} 个节点 · {payload.edges.length} 条连接</p></div>
