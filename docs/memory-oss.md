...
Agent 可以调用 `add_memory` 新增约定、`update_memory` 修改已有条目、`delete_memory` 删除用户指定的条目。成功返回后沿用“记忆已更新”提示；失败不会显示成功。这里不再调用 Mem0 Platform，也不需要 `MEM0_key`；DeepSeek 仍使用现有 API。

修改、删除工具使用检索结果或本会话工具结果中的真实记忆 ID。用户、工作空间、会话身份由服务端运行上下文提供，模型不能传入或更改。Mem0 的按 ID 接口本身不带范围过滤，因此执行前在同一读写锁内核验 user_id、run_id 和 workspace_id；查不到或不属于当前会话时返回 NOT_FOUND。

`update_memory` 用完整新内容替换原条目，保留 ID 和归属，由 SDK 更新向量并记录变更历史。`delete_memory` 删除记忆库条目，不清除聊天记录。成功修改或删除后，当前请求已缓存的旧条目会被移出检索上下文，新内容通过工具结果传给模型，下一轮按正常流程重新检索。临时要求不改长期约定。

SDK 可能在向量存储已改变后写入变更历史失败。因此，通过归属校验并尝试写入后即使返回 FAILED，也会移除该 ID 的本轮召回缓存，避免继续注入旧内容；最终存储状态由后续检索确认，不将这种失败视为已回滚或已成功。
...
- `DATA_AGENT_MEMORY_ENABLED=1`：启用记忆；设为 0 时不注册新增、修改、删除工具，不检索、不注入记忆。
...
```powershell
.\.venv\Scripts\python.exe src/memory/smoke_crud.py
```

该脚本让真实模型选择新增、修改、删除工具，经过生产工具校验和执行节点，在独立目录 `runtime/mem0-crud-smoke/<测试ID>` 验证“7人 → 5人 → 删除”。它核对 ID 保持、重新打开存储后修改仍在、删除后检索为空，以及其他用户/工作空间/会话即使知道 ID 也无法修改或删除。结果记录在该目录的 `result.json`。这是三轮工具集成验证；SQL分析能力仍以已有长对话评测为准。

在网页同一会话中也可以手动输入：

1. 请记住：虚拟偶像回访候选名单最多7人，作为本会话长期约定。
2. 把刚才的长期约定改成最多5人，其他要求不变。
3. 撤销这条候选人数约定，把它从记忆中删除。

可用于简历的实现描述：基于 LangGraph 与 Mem0 OSS 构建 Agent 长期记忆模块，实现会话隔离的增删改查、语义检索注入、动态约定维护及前端状态反馈，并完成多轮工具联调与长上下文 A/B 验证。
